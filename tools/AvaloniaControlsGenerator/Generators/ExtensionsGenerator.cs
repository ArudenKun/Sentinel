    ...
    public void Generate(string outputPath, List<Class> classes)
    {
        foreach (var c in classes)
        {
            ...
            for (var i = 0; i < c.Properties.Length; i++)
            {
                ...
                var template =
                    p.IsReadOnly ? Templates.PropertyMethodsTemplateReadOnly
                    : c.IsSealed ? Templates.PropertyMethodsTemplateSealed
                ...
                if (p.IsEnum && !p.IsReadOnly && p.EnumNames is { })
                {
                    foreach (var enumName in p.EnumNames)
                    {
                        ...
                        propertyEnumBuilder.Replace(
                            "%ClassType%",
                            ReflectoniaFactory.ToString(c.Type)
                        );
                        propertyEnumBuilder.Replace("%Name%", p.Name);
                        propertyEnumBuilder.Replace(
                            "%OwnerType%",
                            ReflectoniaFactory.ToString(p.OwnerType)
                        );
                        propertyEnumBuilder.Replace(
                            "%ValueType%",
                            ReflectoniaFactory.ToString(p.ValueType)
                        );
            ...
            for (var i = 0; i < routedEvents.Length; i++)
            {
                ...
                {
                    Log.Error(
                        $"Routed event {c.Name}.{e.Name} extensions have been already added."
                    );
                    continue;
                }
                ...
                var routes = e.RoutingStrategies.Split(
                    new[] { ',', ' ' },
