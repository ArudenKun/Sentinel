...
namespace AvaloniaControlsGenerator.Generators;
...
public class BuildersGenerator
{
    ...
    )
    {
        ...
        sb.AppendLine(
            $"    /// Creates a new instance of the <see cref=\"{ReflectoniaFactory.FixType(type.ToString())}\"/> class."
        );
        sb.AppendLine("    /// </summary>");
        ...
        {
            sb.AppendLine(
                $"    /// <param name=\"ref\">The reference of the <see cref=\"{ReflectoniaFactory.FixType(type.ToString())}\"/> instantiated class.</param>"
            );
        }
        ...
        {
            sb.AppendLine(
                $"    /// <param name=\"{parameters[j].Name}\">The {parameters[j].Name} value.</param>"
            );
        }

        sb.AppendLine(
            $"    /// <returns>The new instance of the <see cref=\"{ReflectoniaFactory.FixType(type.ToString())}\"/> class.</returns>"
        );
        sb.Append(
            $"    public static {ReflectoniaFactory.FixType(type.ToString())} {ReflectoniaFactory.FixClassNameType(type.Name)}("
        );
        ...
        for (var j = 0; j < parameters.Length; j++)
        {
            ...
            sb.Append(
                $"{ReflectoniaFactory.FixType(parameters[j].ParameterType.ToString())} {parameters[j].Name}"
            );
            ...
            if (parameters[j].HasDefaultValue)
            {
                if (parameters[j].DefaultValue is { })
                {
                    ...
                    {
                        sb.Append(
                            $" = {ReflectoniaFactory.FixType(parameters[j].ParameterType.ToString())}.{parameters[j].DefaultValue}"
                        );
                    }
                    else
                    {
