...
using AvaloniaControlsGenerator.Reflectonia.Model;

namespace AvaloniaControlsGenerator.Reflectonia;
...
public class ReflectoniaFactory
{
    ...
    {
        return type is null ? string.Empty : FixType(type.ToString());
    }
    ...
    {
        return t.Replace("`1", "").Replace("`2", "").Replace("`3", "").Replace("+", "");
    }
    ...
    )
    {
        ...
        if (
            registry.RegisteredProperties is null
        ...
        foreach (var classType in classTypes)
        {
            ...
            classes.Add(c);
            Log.Info(
                $"The `{classType.Name}` class has {properties.Count} properties and {events.Count} events."
            );
        }

        Log.Info(
            $"Found {classes.Count} classes with total of {classes.SelectMany(x => x.Properties).Count()} properties and {classes.SelectMany(x => x.Events).Count()} events."
        );
    ...
    private Type[] GetClassTypes(Predicate<Assembly> assemblyFilter, Predicate<Type> typeFilter)
    {
        return (
            ...
            where assembly?.FullName is not null && assemblyFilter(assembly)
            from assemblyType in assembly.GetTypes()
            where
                assemblyType is not null
                && typeFilter(assemblyType)
                && assemblyType.IsSubclassOf(typeof(AvaloniaObject))
                && assemblyType
                    .GetCustomAttributesData()
                    .All(x => x.AttributeType.Name != "ObsoleteAttribute")
    ...
    private List<Property> GetProperties(Type classType, ReflectoniaRegistry registry)
    {
        ...
        foreach (var property in avaloniaProperties)
        {
            ...
            {
                Log.Info(
                    $"The `{classType.Name}.{propertyName}Property` property type `{property.PropertyType.Name}` is not public."
                );
                continue;
            }
            ...
            if (property.IsAttached && registry.RegisteredAttachedProperties is { })
            {
                foreach (var kvp1 in registry.RegisteredAttachedProperties)
                {
                    foreach (var kvp2 in kvp1.Value)
                    {
                        ...
                        {
                            Log.Info(
                                $"Attached property `{classType.Name}.{propertyName}Property` registered owner type changed from `{ownerType.Name}` to `{kvp1.Key.Name}`."
                            );
                            ownerType = kvp1.Key;
                        }
                    }
            ...
            if (property.OwnerType != classType)
            {
                ...
                Log.Info(
                    $"Attached property `{classType.Name}.{propertyName}Property` registered owner type changed from `{ownerType.Name}` to `{classType.Name}`."
                );
                ownerType = classType;
            }
    ...
    private List<Event> GetEvents(Type classType, ReflectoniaRegistry registry)
    {
        ...
        foreach (var routedEvent in avaloniaRoutedEvents)
        {
            ...
            {
                Log.Info(
                    $"The` {classType.Name}.{eventName}Event` args type `{routedEvent.EventArgsType.Name}` is not public."
                );
                continue;
            }
        ...
        foreach (var eventInfo in eventInfos)
        {
            ...
            {
                Log.Info(
                    $"The `{classType.Name}.{eventName}` event handler type `{eventHandlerType.Name}` is not public."
                );
                continue;
            }
            ...
            {
                Log.Info(
                    $"The `{classType.Name}.{eventName}` event handler type `{eventHandlerType.Name}` arguments `{argsType.Name}` are not public."
                );
                continue;
            }

            var e = new Event(eventName, classType, argsType, eventHandlerType, null);
            events.Add(e);
