...
public class GeneratorHost(ExtensionGeneratorConfig config)
{
    ...
    private void GenerateExtensions()
    {
        ...
        foreach (var controlType in config.TypesToProcess)
        {
            ...
            var extensionGroups = groupGenerators
                .Select(x =>
                {
            ...
            sb.AppendLine(
                $@"[global::System.CodeDom.Compiler.GeneratedCode(""AvaloniaExtensionGenerator"", ""{versionString}"")]"
            );
            ...
            var outputPath = Path.Combine(
                config.OutputPath,
                $"{controlType.Name}_MarkupExtensions.g.cs"
            );
    ...
    private static bool IsAvaloniaPropertyField(FieldInfo field)
    {
        ...
        if (
            field.FieldType.Name.StartsWith("DirectProperty")
    ...
    private static bool IsReadOnlyField(FieldInfo field)
    {
        try
        {
            ...
            var propInfo = GetSingleProperty(controlType, propertyName);
            if (propInfo != null)
            {
    ...
    private static PropertyInfo? GetSingleProperty(Type type, string name)
    {
        // Get *all* properties with that name
        ...
        // 🔹 Resolve ambiguity: pick the one with no index parameters (or whatever criteria you need)
        return props.FirstOrDefault(p => p.GetIndexParameters().Length == 0) ?? props[0];
    }
}
