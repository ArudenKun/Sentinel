...
internal abstract class AttributeMemberGenerator<TAttribute, TDeclarationSyntax>
    : IIncrementalGenerator
    where TAttribute : Attribute
    where TDeclarationSyntax : MemberDeclarationSyntax
{
    protected static string AttributeType => typeof(TAttribute).Name;
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        static bool IsSyntaxTarget(SyntaxNode node, CancellationToken _)
        {
            ...
            bool HasAttributeType()
            {
                ...
                return type.AttributeLists.Any(attributeList =>
        ...
        )
        {
            try
            {
                foreach (var node in nodes.Distinct())
                {
                    ...
                    error ??= DiagnosticDetail.Empty;

                    if (generatedCode is null)
                    {
                        var descriptor = new DiagnosticDescriptor(
                            string.IsNullOrWhiteSpace(error.Id) ? AttributeName : error.Id,
                            error.Title,
                            error.Message,
                            string.IsNullOrWhiteSpace(error.Category) ? "Usage" : error.Category,
    ...
    )
    {
        try
        {
            return GenerateCode(compilation, node, symbol, attributeData, options);
        }
        catch (Exception e)
        {
            // Log.Error(e);
    ...
    private static char[] InvalidFileNameChars { get; } =
    [
        '\"',
        '<',
        '>',
