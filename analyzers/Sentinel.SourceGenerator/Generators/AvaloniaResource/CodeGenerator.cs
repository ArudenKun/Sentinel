...
public static class CodeGenerator
{
    public static string GenerateResource(string @namespace, string modifier)
    {
        ...
        classBuilder
            ...
            .SetType("ConcurrentDictionary<Assembly, Uri>")
            .MakeStatic()
            .WithReadonlyValue("new ConcurrentDictionary<Assembly, Uri>()");
        ...
        classBuilder
            ...
            .WithBody(writer =>
            {
                ...
                {
                    writer.AppendLine(
                        "var assetUri = AssetCache.GetOrAdd(assembly, keyAssembly => AssetLoader"
                    );
                    writer.AppendLine(
                        ".GetAssets(new Uri($\"avares://{keyAssembly.GetName().Name}\"), null)"
                    );
                    writer.AppendLine(
                        ".Single(s => s.AbsoluteUri.EndsWith($\"{FileName}\", StringComparison.InvariantCultureIgnoreCase))) "
                    );
                    writer.AppendLine(
